...
`Write` access to a store is limited to [`WebServer Script`](/concepts/scripts), [`WebsocketServer Scripts`](/concepts/scripts) or `Player` with `Admin` permissions, ensuring the security and integrity of the data.
...
This is table with different scopes scenarios for a player accessing a store:
