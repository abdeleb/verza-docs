...
:::info Scope Access
You can make the store private by setting the scope subfix to `private`. This will make the store only accessible by `Admin Players` and `Server Scripts`.

To learn more about scopes access, see [Scopes](/concepts/storage#access).
:::
